...
(* This is kind of ugly. We brute force try to load every name libcmark might be under. Passing the
   linking flags to the compiler doesn't seem to work reliably, so we try to load dynamically here.
   See https://github.com/ocamllabs/ocaml-ctypes/issues/71 .*)
let libnames = ["libcmark.so"; "libcmark.dylib"]

let _ =
  let rec loop = function
    | n :: ns -> (
        try ignore @@ Dl.dlopen ~filename:n ~flags:[Dl.RTLD_LAZY; Dl.RTLD_GLOBAL] with
        | Dl.DL_error _ -> loop ns )
    | [] -> ()
  in
  loop libnames
...
let parse_file' = foreign "cmark_parse_file" (ptr void @-> int @-> returning (ptr void))
...
let of_file ?(flags : parse_flag list = []) path =
  ...
  if fh = null then
    ...
    if res <> 0 then
      `Error "fclose: Failed to close file."
    else
      `Ok (finalized { node })
...
let render_xml' = foreign "cmark_render_xml" (ptr void @-> int @-> returning string)

let render_html' = foreign "cmark_render_html" (ptr void @-> int @-> returning string)

let render_man' = foreign "cmark_render_man" (ptr void @-> int @-> int @-> returning string)

let render_commonmark' =
  foreign "cmark_render_commonmark" (ptr void @-> int @-> int @-> returning string)


let render_latex' = foreign "cmark_render_latex" (ptr void @-> int @-> int @-> returning string)

let renderer prim ?(flags : render_flag list = []) { node } = prim node (int_of_flags flags)

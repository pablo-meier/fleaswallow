...
let build_blog_model path =
  ...
  let template_path file =
    List.fold_left ~init:path ~f:Filename.concat ["templates"; file] in
  let template_string x = template_path x |> Files.file_contents |> Option.value_exn |> Files.contents in

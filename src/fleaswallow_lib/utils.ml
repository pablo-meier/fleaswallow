...
let option_maybe x = match x with Some v -> v | None -> "None"
...
(** Don't want to call Cmark.html_from_string directly because its safety flag,
    which * removes my inline HTML (titles on legacy posts, a lot of images) *)
...
(** Get up to 300 characters of words from the first non-header in a body of
    Markdown. *)
let rec get_desc lst =
  match lst with
  | [] -> ""
  | x :: xs -> ( match x with "" -> get_desc xs | _ -> x )
...
(** Sub out anything that isn't a number, letter, or dash. Lowercase all.
    Dashes where spaces lived. *)
let dasherized name =
...
let%test "dasherized basic" =
    dasherized "Hark! A meadow"  = "hark-a-meadow"

let%test "dasherized empty string" =
    dasherized ""  = ""

let%test "dasherized multiple odd chars" =
    dasherized "Pablo*&^%$#"  = "pablo"

...
type post_body = Split of (string list * string list) | Whole of string list
...
let filename_regex : Re2.regex =
  Re2.create_exn ".+/\\d\\d\\d\\d-\\d\\d-\\d\\d-([a-z-0-9]+)\\.md$"
...
(** Doing the naive thing Medium stated they did way back when: "Read time
    is based on the average reading speed of an adult (roughly 275 WPM). We
    take the total word count of a post and translate it into minutes. Then, we
    add 12 seconds for each inline image. Boom, read time." Note that
    Omd used to let us have the structure as Markdown elements, so we could
    go _into_ certain structures like H1 or paragraphs. The move to Cmark means
    we're dealing with raw strings here, so things like [links][1] will still
    parse as one word, but the `>` behind a blockquote might also. Inline HTML,
    like we do for image tags might also cause a problem. It's…
    imperfect, and a FIXME. But I'd rather have Cmark than Omd's weird
    rendering so we'll do naive string split. *)
let reading_time_for content =
  let num_words str =
    ...
    |> List.length
  in
  content |> all_body |> Utils.add_newlines |> num_words
  |> Pervasives.float_of_int
  |> (fun x -> x /. 275.0)
  |> Pervasives.ceil |> Pervasives.int_of_float

(** Given a bunch of non-metadata lines, returns it as either Whole or Split.
    We have to * do the Markdown parsing as whole since the links at the top
    might refer to links * at the bottom, so we pattern match the split later. *)
let handle_content contents =
  let is_separator x = String.strip x <> "<!-- more -->" in
  contents |> List.split_while ~f:is_separator |> function
  | top, [] -> Whole top
...
(** At a high level, we go iterate through lines. First we take metadata *
    ...
    until we optionally match the "<!-- more" signal, after which we * add to
    the end. *)
...
(** Go through all your lines and eventually construct a tuple. This is
    designed inefficiently * but it's such a small amount of data anyways... *)
let handle_metadata data =
  ...
  in
  let to_datetime =
    Fn.compose Unix.gmtime (ISO8601.Permissive.datetime ~reqtime:true)
  in
  let title = metadata_handle "Title" |> Option.value_exn in
  let datetime = metadata_handle "Date" |> Option.value_exn |> to_datetime in
  let tags =
    metadata_handle "Tags" |> Option.value_exn |> String.split ~on:','
...
let compare_post_dates { datetime = datetime1; _ } { datetime = datetime2; _ }
  ...
  match tm datetime1 < tm datetime2 with true -> 1 | _ -> -1
...
let form_prev_next_links posts =
  ...
  | false -> posts
  | true ->
      List.fold_left ~f:foldable ~init:(0, []) posts |> Utils.snd |> List.rev
...
let post_to_string
  ...
  Printf.sprintf
    "Post(title: %s; time: %s; tags: %s; og_image: %s; og_desc: %s; content: \
     %s; reading_time: %d; next %s; prev %s)"
    title time_str tag_string og_img og_desc
    (* content_str *) "CONTENT CONTENT" reading_time next prev

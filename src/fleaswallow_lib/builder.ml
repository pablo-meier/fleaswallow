...
let tag_url tag =
  Jg_types.Tobj
    [ ("name", Jg_types.Tstr tag); ("url", Jg_types.Tstr (tag_path tag)) ]
...
let index_post_model model post =
  ...
  let publish_date_iso =
    Post.datetime post |> Unix.mktime |> Utils.fst
    |> ISO8601.Permissive.string_of_datetime
  in
  let description =
    Post.og_description post |> or_string ~default:(Utils.get_desc content_md)
  in
...
let index_model model tag posts =
  ...
  let full_atom_uri =
    String.concat ~sep:"/" [ Model.hostname model; atom_path_for tag ]
  in
...
let generate_index_pages model =
  ...
  in
  Model.posts_by_tag model |> Map.to_alist
...
let generate_post_pages model =
  ...
  let prev_next_url = function
    ...
    | Some (title, path) ->
        Jg_types.Tobj
          [ ("name", Jg_types.Tstr title); ("url", Jg_types.Tstr path) ]
  in
  let make_model post =
    let title, reading_time = (Post.title post, Post.reading_time post) in
    let content_md = Post.all_content post in
    let description =
      Post.og_description post
      |> or_string ~default:(Utils.get_desc content_md)
    in
    let og_image =
      Post.og_image post |> or_string ~default:(Model.default_og_image model)
    in
    ...
    let content =
      content_md |> Utils.add_newlines |> Utils.cmark_from_string
    in
    let prev = Post.prev_post_fs_path post in
    let next = Post.next_post_fs_path post in
    let author = Model.author model in
    let keywords_list =
      Post.tags post |> String.concat ~sep:" " |> ( ^ ) (Model.author model)
    in
  ...
  let post_page p =
    ...
    Jg_template.from_string ~models template_string
  in
  all_posts model |> List.map ~f:(fun x -> (Post.fs_path x, post_page x))
...
let generate_homepage model =
  ...
  let homepage_model posts =
    let tags =
      model |> Model.posts_by_tag |> Map.to_alist
      |> List.map ~f:(Fn.compose tag_url Utils.fst)
    in
    [
  ...
  let render_homepage posts =
    ...
    [ ("index.html", contents) ]
  in
  model |> all_posts |> take 5 |> render_homepage
...
let generate_statics model =
  ...
  let statics_model page =
    ...
    let contents = Page.contents page |> Utils.cmark_from_string in
    [
      ("title", Jg_types.Tstr title);
      ("author", Jg_types.Tstr (Model.author model));
      ("description_abridged", Jg_types.Tstr description);
  ...
  let render_static page =
    ...
    let base_name =
      match
        Page.fs_path page |> Filename.basename |> Filename.split_extension
      with
    ...
    (base_name, contents)
  in
  Model.static_pages model |> List.map ~f:render_static

(** Generates the blog's RSS feed. Generate an "all" feed to start, * later one
    for every tag. * *)
let generate_rss_feeds model =
  ...
  let make_rss_from models =
    Jg_template.from_string ~models rss_template_string
  in
  let make_atom_from models =
    Jg_template.from_string ~models atom_template_string
  in
  let make_feeds_for (tag, posts) =
    let rss_filename = rss_path_for tag in
    let atom_filename = atom_path_for tag in
    let models =
      index_model model tag (List.take posts @@ Model.posts_in_feed model)
    in
    [
  ...
  in
  Model.posts_by_tag model |> Map.to_alist |> List.map ~f:make_feeds_for
  |> List.concat
...
let generate_sitemap model pairs =
  ...
  let hostname = Model.hostname model in
  let contents =
    pairs |> List.map ~f:Utils.fst
    |> List.map ~f:(Filename.concat hostname)
    |> List.cons hostname |> Utils.add_newlines
  in
...
let build model =
  ...
  |> List.concat |> generate_sitemap model

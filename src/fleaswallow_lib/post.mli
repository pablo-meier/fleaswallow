...
(** * The bread-and-butter of any blog is a post. This module contains a
    datatype to create * and manipulate posts. **)
...
(** Get required values to link to the next post, if applicable. The first is
    the title, * the second is the filesyste path (used to determine the URL). *)

val prev_post_fs_path : t -> (string * string) option
(** Get required values to link to the previous post, if applicable. The first
    is the title, * the second is the filesyste path (used to determine the
    URL). *)
